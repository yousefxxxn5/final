<div>
    ...
    <li class="nav-item dropdown notification_dropdown">
        ...
        <a class="nav-link" href="javascript:void(0);" id="referenceLink">
        ...
        @script
            <script>
                ...
                window.addEventListener('load', function() {
                    targetDiv.style.display = 'none'; // إخفاء العنصر عند التحميل
                });

                // تبديل الإظهار/الإخفاء عند النقر على الرابط
                referenceLink.addEventListener('click', function() {
                    const phpValue = "{{ $state }}";
                    if (targetDiv.style.display == 'none')
                        $wire.dispatch('click_ok');
                    targetDiv.style.display = (targetDiv.style.display === 'block') ? 'block' : 'block';
                });

                // إخفاء العنصر عند النقر خارج الرابط والعنصر الهدف
                document.addEventListener('click', function(event) {
                    if (!referenceLink.contains(event.target) && !targetDiv.contains(event.target)) {
                        targetDiv.style.display = 'none'; // إخفاء العنصر الهدف
                    }
                });
            </script>
        @endscript
